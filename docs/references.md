...
Canonical documentation for the directory standards that tooldirs implements.
...
Microsoft's system for identifying standard folders via unique identifiers. Replaces the older CSIDL constants. tooldirs uses `SHGetKnownFolderPath` to resolve paths.
...
BSD-specific file system hierarchy documentation. While tooldirs follows XDG on BSD platforms, hier(7) provides context for system-wide paths.
