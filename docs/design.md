# tooldirs design document

This document describes the design decisions, behavioral semantics, and configuration options for the tooldirs package.

## Overview

tooldirs resolves platform-appropriate filesystem paths for app data. The library addresses a fundamental tension: apps want portable code, but users expect platform-native behavior. A settings file "should" live in `~/Library/Application Support` on macOS, `%APPDATA%` on Windows, and `~/.config` on Linux.
...
dirs, err := tooldirs.NewWithConfig(tooldirs.Config{
    AppName:  "testapp",
    Platform: tooldirs.PlatformLinux, // Works on any OS
})
```
...
Logical semantics match user expectations and align with how most tools behave (git, cargo, npm all use logical paths). Users think in terms of their mental model of the filesystem, not where bytes physically reside. Physical traversal would produce surprising results when symlinks cross mount points or reference paths outside the user's home directory.
...
The `adrg/xdg` library provides platform-native defaults on macOS (Library directories) and Windows (Known Folders) when XDG environment variables are not set. The key architectural differences from tooldirs are:

- Global singleton API with package-level variables (`xdg.ConfigHome`, `xdg.DataHome`) versus tooldirs' instance-based API with configuration options
...
tooldirs handles app-name-scoped paths, versioning, and layered directory search automatically. Use `XDGOnAllPlatforms` if you want XDG paths as the primary location on all platforms regardless of native conventions.
...
tooldirs builds on the same platform conventions while providing a complete solution for app directory management.
