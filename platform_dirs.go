...
var ErrAppNameRequired = errors.New("toolpaths: AppName is required")
...
// userHomeDirFunc is the function used to get the home directory.
// It can be overridden in tests via SetHomeDirFunc.
var userHomeDirFunc = defaultUserHomeDir

// homeDirFuncOverridden tracks whether SetHomeDirFunc was called with a non-nil value.
// This is used by Windows functions to bypass system calls during tests.
var homeDirFuncOverridden bool

// IsHomeDirFuncOverridden returns true if SetHomeDirFunc has been called
// with a non-nil function. This is used internally by platform-specific code.
func IsHomeDirFuncOverridden() bool {
	return homeDirFuncOverridden
}

// SetHomeDirFunc sets a custom function to resolve the user's home directory.
// This is intended for testing. Pass nil to restore the default behavior.
// The function is not safe for concurrent use with other calls to this package.
func SetHomeDirFunc(f func() string) {
	if f == nil {
		userHomeDirFunc = defaultUserHomeDir
		homeDirFuncOverridden = false
	} else {
		userHomeDirFunc = f
		homeDirFuncOverridden = true
	}
}

// userHomeDir returns the current user's home directory using the
// configured function (defaultUserHomeDir or a test override).
func userHomeDir() string {
	return userHomeDirFunc()
}

// defaultUserHomeDir returns the current user's home directory.
// On Unix systems, this checks $HOME first for testability, then falls back
// to os.UserHomeDir(). On Windows, it uses os.UserHomeDir() directly.
...
func (d *PlatformDirs) String() string {
	...
	b.WriteString("toolpaths.PlatformDirs{\n")
