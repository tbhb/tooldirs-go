...
func TestNewAppNameRequired(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dirs, err := toolpaths.New(tt.appName)
			require.ErrorIs(t, err, toolpaths.ErrAppNameRequired)
			assert.Nil(t, dirs)
		})
	}
}
...
func TestXDGEnvOverrides(t *testing.T) {
	tests := []struct {
		name   string
		envVar string
		getDir func(*toolpaths.PlatformDirs) string
	}{
		{
			name:   "XDG_CONFIG_HOME override",
			envVar: "XDG_CONFIG_HOME",
			getDir: (*toolpaths.PlatformDirs).UserConfigDir,
		},
		{
			name:   "XDG_DATA_HOME override",
			envVar: "XDG_DATA_HOME",
			getDir: (*toolpaths.PlatformDirs).UserDataDir,
		},
		{
			name:   "XDG_CACHE_HOME override",
			envVar: "XDG_CACHE_HOME",
			getDir: (*toolpaths.PlatformDirs).UserCacheDir,
		},
		{
			name:   "XDG_STATE_HOME override",
			envVar: "XDG_STATE_HOME",
			getDir: (*toolpaths.PlatformDirs).UserStateDir,
		},
	}
...
func TestFindConfigFileNotFound(t *testing.T) {
	dirs, err := toolpaths.New("testapp-notexist-12345")
	require.NoError(t, err)
...
func TestPlatformString(t *testing.T) {
	tests := []struct {
		platform toolpaths.Platform
		expected string
	}{
		{toolpaths.PlatformAuto, "auto"},
		{toolpaths.PlatformLinux, "linux"},
		{toolpaths.PlatformMacOS, "macos"},
		{toolpaths.PlatformWindows, "windows"},
		{toolpaths.PlatformFreeBSD, "freebsd"},
		{toolpaths.PlatformOpenBSD, "openbsd"},
	}
...
func TestPlatformOverride(t *testing.T) {
	...
	t.Run("PlatformAuto triggers detection", func(t *testing.T) {
		...
		dirs, err := toolpaths.NewWithConfig(toolpaths.Config{
			AppName:  "testapp",
			Platform: toolpaths.PlatformAuto,
		})
		require.NoError(t, err)
